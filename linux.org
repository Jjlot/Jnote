...
*** file <--> image
docker save tempest | gzip > tempest.tar.gz

docker load --input tempest.tar.gz
...
** Encapsulation
*** Origin packet
| Origin IP header | TCP header | Data |

*** AH transport mode
| Origin IP header | AH header | TCP header | Data |

*** AH tunnel mode
| New IP header | AH header | Origin IP header | TCP header | Data |

*** ESP transport mode
| Origin IP header | ESP header | TCP header | Data | ESP trailer | ESP Authorization |

*** ESP tunnel mode
| New IP header | ESP header | Origin IP header | TCP header | Data | ESP trailer | ESP Authorization |
...
** 野蛮模式
ike v1协商的phase 1用到的一种模式，与之对应的是main模式
...
软件工程师需要看到一个没有硬件的纯软件世界，硬件必须透明的提供给他，谁来实现这一任务？答案是驱动程序，驱动程序从字面解释就是：“驱使硬件设备行动”。

linux内核分为5大部分：多任务管理、内存管理、文件系统管理、设备管理、网络管理；


*** 设备类型
Linux把设备驱动程序分为4种：字符设备，块设备，网络设备及其他设备。(不确定是不是有其他设备)

linux中所有的驱动程序最终都能归到这三种设备中
...
*** 设备号
主设备号用来表示一个特定的驱动程序。 次设备号用来表示使用该驱动程序的各设备。 例如一个嵌入式系统，有两个LED指示灯，LED灯需要独立的打开或者关闭。 
...
*** 
https://zhuanlan.zhihu.com/p/154288298
