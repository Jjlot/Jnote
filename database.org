...
select device_id from ports where device_owner = 'network:floatingip' order by device_id DESC;
...
*** complacate conditions
SELECT * FROM students WHERE (score < 80 OR score > 90) AND gender = 'M';

*** 分页查询
# 从1号记录开始，最多取10条
select * from routerports limit 10 offset 1;
# 简写
select * from routerports limit 1, 10;

*** count group
select network_id, count(*) from ports group by network_id;
...
select p.id port_id, p.name port_name, p.network_id, n.name network_name from ports p inner join networks n on p.network_id = n.id limit 30;
